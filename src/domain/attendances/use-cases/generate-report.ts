...
import { EmployeeNotFoundError } from "../../absence-allowance/errors/employee-not-found-error";
import { NotFoundEmployeeReportError } from "../errors/not-found-employee-report-error";
...
type GenerateReportUseCaseResponse = Either<
	EmployeeNotFoundError | NotFoundEmployeeReportError | null,
	{
		reports: Reports;
	}
>;
...
export class GenerateReportUseCase implements IUseCase<IGenerateReportUseCaseDTO, GenerateReportUseCaseResponse> {
	...
	async execute({ rfid, startDate, endDate }: IGenerateReportUseCaseDTO): Promise<GenerateReportUseCaseResponse> {
		...
		if (!attendance) {
			return left(new NotFoundEmployeeReportError(employee.rfid));
		}
		...
		workDaysAndSchedules = attendance.map((record) => ({
			date: record.date,
		...
		const absenceAllowanceDates = new Set(
			absenceAllowance.map((absenceAllowance) => dayjs(absenceAllowance.date).format("YYYY-MM-DD")),
		);
		if (!absenceAllowance) {
			return left(null);
		}
