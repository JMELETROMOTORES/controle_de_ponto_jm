...
class GenerateReportController implements IController {
	...
	async handle(
		...
		try {
			...
			if (result.isLeft()) return response.status(HttpStatusCode.BAD_REQUEST).json(result.value);

			return response.status(HttpStatusCode.OK).json(ReportPresenter.toHTTP(result.value?.reports));
		} catch (error) {
			next(error);
		}
	}
